...
export function ProviderSettingsDialog() {
  ...
  }, [providerSettingsDialogOpen, fetchProviders])

  // Only video providers — image is upload only
  const filteredProviders = providers.filter((p) => p.type === 'video')
  const accentColor = 'var(--col-video)'
  ...
  const handleEdit = useCallback((provider: Provider) => {
    ...
    setForm({
      ...
      endpoint: provider.endpoint || '/v1/videos/generations',
      model: provider.model,
      api_key: provider.api_key || 'local',
    ...
    try {
      if (editingId && !isNew) {
        const res = await fetch(`/api/storyboard/providers/${editingId}`, {
          ...
          body: JSON.stringify({ ...form, type: 'video' }),
        })
        const data = await res.json()
        if (data.ok) {
  ...
  }, [form, editingId, isNew, fetchProviders])
      ...
      <Dialog open={providerSettingsDialogOpen} onOpenChange={setProviderSettingsDialogOpen}>
        <DialogContent className="sm:max-w-3xl max-h-[85vh] flex flex-col overflow-hidden">
          <DialogHeader className="shrink-0">
            ...
            <DialogDescription>
              Configure the video generation provider (WAN 2.2 i2v).
              Images are uploaded by the user, so no image provider is needed.
            </DialogDescription>
          </DialogHeader>

          <div className="mt-2 flex gap-4 min-h-0 flex-1 overflow-hidden">
            {/* Provider List */}
            <div className="w-60 shrink-0 flex flex-col min-h-0 overflow-hidden">
              <div className="flex items-center justify-between mb-2 shrink-0">
                <span className="text-xs text-muted-foreground">
                  {filteredProviders.length} video provider
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setForm((f) => ({
                              ...f,
                              timeout_seconds: parseInt(e.target.value) || 1800,
                            }))
                          }
                        />
                      </div>
          ...
          <DialogFooter className="shrink-0">
            {isEditing && (
              <>
                <Button variant="outline" className="h-9 text-xs" onClick={handleCancel}>
                  Cancel
                </Button>
                {editingId && !isNew && (
                  <Button
