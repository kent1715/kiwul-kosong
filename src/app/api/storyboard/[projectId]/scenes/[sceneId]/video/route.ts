...
import { saveVideoFile, ensureProjectDirs, clearSceneVideo } from '@/lib/file-storage'
...
function extractVideoFromProviderResponse(data: any): string {
  ...
  if (!value || typeof value !== 'string') {
    throw new Error(
      `Unexpected video provider response format: ${JSON.stringify(data).slice(0, 1000)}`
    )
  }
...
async function pollVideoUntilReady(
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    for (const pollUrl of pollUrls) {
      try {
        ...
        console.log(
          `[video poll] task=${taskId} status=${data?.status || data?.state} progress=${progress}`
        )
        break
      } catch (err) {
  ...
  throw new Error(
    `Video generation timeout waiting for task ${taskId}. Last response: ${JSON.stringify(data).slice(0, 1000)}`
  )
}
...
export async function POST(
  ...
  try {
    ...
    // Reset previous video state before starting a new generation
    try {
      await clearSceneVideo(projectId, sceneId)
    ...
    // Check that an image exists (WAN i2v needs an input image)
    if (scene.image_status !== 'completed' || !scene.image_path) {
      return NextResponse.json(
        {
          ok: false,
          error: 'Image must be uploaded before generating video',
        },
        { status: 400 }
      )
    }
    ...
    try {
      ...
      const projectRow = await db.storyboardProject
      ...
      const dataUrl = await imagePathToDataUrl(scene.image_path || '', request.url)
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          // Send frame count with several possible field names because
          // different WAN providers use different keys. fps 16 -> 5 sec = 81 frames
          num_frames: scene.duration * 16 + 1,
          frames: scene.duration * 16 + 1,
          length: scene.duration * 16 + 1,
          video_length: scene.duration * 16 + 1,
          total_frames: scene.duration * 16 + 1,
          frame_count: scene.duration * 16 + 1,
          quality: 'standard',
          preset: 'standard',
          width: videoWidth,
          height: videoHeight,
